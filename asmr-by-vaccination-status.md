...
<p>
In other words: non-overlapping 95% confidence intervals for two groups (where the vertical error bars do not overlap) signify that 19 times out of 20 the lack of overlap signals we would be right in thinking there is a real difference between the groups (or more frequently than 19 times out of 20 i.e. if the bars were nowhere near touching then we would almost always be right about the groups really being different whereas when the vertical error bars for the 95% confidence interval are very close without quite touching the 19 times out of 20 idea is a more relevant approximation of how we would be wrong if saying bars like that, that did not overlap, meant there was a real, non-random, difference between the groups). Whereas when the confidence intervals overlap we would be less sure of being right if we assert the two groups are different - rather than the difference we are seeing in the sample being a product of the same underlying group value with random variation being the cause of the apparent difference.
<p>
